...
3. A 执行 `/language` 切换到 English，确认主菜单、积分、签到、商城和抽奖常用提示变成英文；再切回中文。
4. A 发送 `/invite`，复制专属链接给此前从未启动过该测试机器人的 B。
5. B 从邀请链接启动，但不加入频道，执行 `/verify`，应提示尚未加入且 A 不加分。
6. B 加入所有频道再验证，应通过；A 增加一次邀请奖励。
7. B 再次验证、反复点击验证按钮，A 的奖励不能重复增加；高频点击应触发冷却提示。
8. A 连续执行两次 `/checkin`，只允许第一次加分。
9. 管理员运行 `/addproduct 10 测试卡`，再用本地文件导入测试卡密：

   ```powershell
   python -m scripts.import_cards 1 .\test-codes.txt
   ```

10. 给 A 足够积分：`/addpoints A的数字ID 20`。在 `/shop` 中确认兑换并快速重复点击，只能扣一次积分、发一条库存卡密。
11. A 运行 `/mycards`，应能找回兑换记录；卡密消息应带 Telegram 内容保护。
12. 在群里发送 `/points`，机器人只提示转到私聊，不泄露积分或卡密。
13. 管理员在群里发起付费卡密群抽奖，例如 `/grouplottery product 1 1 time 1m cost 2 兔姐666 测试付费卡密抽奖`；A 发送 `兔姐666` 后应扣 2 积分并进入名单，积分不足账号不能参与，开奖后中奖卡密只私聊发放。
14. 管理员检查 `/stats`、`/products`、上下架商品。用 `/addcards` 直接粘贴卡密时，确认原消息已删除；生产环境优先用本地导入脚本。
15. 停止进程后再次启动，确认积分、邀请、库存、卡密和语言偏好仍在。
